//! Unicode category ranges, generated from llama.cpp b10090's
//! `unicode-data.cpp` (scripts/gen-unicode-data.py output) so that the
//! pretokenizer classifies codepoints exactly like the reference
//! implementation. Half-open is not used: ranges are inclusive.
...
/// `\p{N}` — any numeric character, not just ASCII digits.
pub fn is_number(c: char) -> bool {
    in_ranges(NUMBER, c)
}

/// `\s` — the White_Space set llama.cpp uses, not Rust's `char::is_whitespace`
/// (which agrees today, but the table is what was validated against).
pub fn is_whitespace(c: char) -> bool {
    in_ranges(WHITESPACE, c)
}

pub const LETTER: &[(u32, u32)] = &[
